...
var GP_PROCESS_NAME = "ProcessDesenvolvimentoProjetos";
...
function afterTaskCreate(colleagueId) {
    try {
        ...
        log.error("[GP][ProcessDesenvolvimentoProjetos.afterTaskCreate] " + e);
    }
}

function getNotifyConfig(activity) {
    var configs = {
        "14": { name: "Realizar Planejamento do Projeto", type: "group", target: "TI", label: "TI", route: "projectPlanning" },
        "18": { name: "Execução do Projeto", type: "group", target: "TI", label: "TI", route: "projectExecution" },
        "23": { name: "Solicitante - Validação do Projeto", type: "user", field: "solicitanteColleagueIdNS", route: "projectRequesterValidation" },
        "32": { name: "TI - Validação do Projeto", type: "group", target: "TI", label: "TI", route: "projectTiValidation" },
        "46": { name: "Tratar Erro - Integrar GLPI", type: "group", target: "TI", label: "TI", route: "dpGlpiErrorTreatment" },
        "47": { name: "Tratar Erro - Iniciar Execução", type: "group", target: "TI", label: "TI", route: "dpStartExecErrorTreatment" },
        "52": { name: "Tratar Erro - Integrar GLPI", type: "group", target: "TI", label: "TI", route: "dpGlpiErrorTreatment" },
        "56": { name: "Tratar Erro - Iniciar Entrega", type: "group", target: "TI", label: "TI", route: "dpStartDeliveryErrorTreatment" }
    };
...
function getSubstituteContent(rawResponse) {
    try {
        ...
        log.warn("[GP][ProcessDesenvolvimentoProjetos.afterTaskCreate] Erro ao ler substitutos: " + e);
        return [];
    }
}
...
function sendNotification(context, recipient) {
    ...
    log.info("[GP][ProcessDesenvolvimentoProjetos.afterTaskCreate] notifier.notify destino=" + recipient.receiver +
