...
var GP_PROCESS_NAME = "execucaoFasesAtividades";
...
function afterTaskCreate(colleagueId) {
    try {
        ...
        log.error("[GP][execucaoFasesAtividades.afterTaskCreate] " + e);
    }
}

function getNotifyConfig(activity) {
    var configs = {
        "14": { name: "Aguardando Execução da Atividade", type: "group", target: "TI", label: "TI", route: "executionActivityWaiting" },
        "18": { name: "Execução da Atividade", type: "group", target: "TI", label: "TI", route: "executionActivity" },
        "23": { name: "Solicitante - Validação da Atividade", type: "user", field: "solicitanteColleagueIdNS", route: "executionActivityRequesterValidation" },
        "32": { name: "TI - Validação da Atividade", type: "group", target: "TI", label: "TI", route: "executionActivityTiValidation" },
        "46": { name: "Tratar Erro - Integrar GLPI", type: "group", target: "TI", label: "TI", route: "efGlpiErrorTreatment" },
        "52": { name: "Tratar Erro - Integrar GLPI", type: "group", target: "TI", label: "TI", route: "efGlpiErrorTreatment" }
    };
...
function getSubstituteContent(rawResponse) {
    try {
        ...
        log.warn("[GP][execucaoFasesAtividades.afterTaskCreate] Erro ao ler substitutos: " + e);
        return [];
    }
}
...
function sendNotification(context, recipient) {
    ...
    log.info("[GP][execucaoFasesAtividades.afterTaskCreate] notifier.notify destino=" + recipient.receiver +
