function createDataset(fields, constraints, sortFields) {
	...
	var usuario = "svc.totvs.fluig";
	var senha = "5ZsRsaj583zf";
	var CODCOLIGADA = "1";
	var context = "CODUSUARIO=svc.totvs.fluig;CODSISTEMA=V;CODCOLIGADA=1"
	var filtro = "TCPg.INATIVO = 0 AND TCPg.PLANOCOMPRA = '1' ";

    var created = false;

	if (constraints != null) {
		for (var c = 0; c < constraints.length; c++) {
			if (constraints[c].fieldName == "CODCOLIGADA") {
				CODCOLIGADA = constraints[c].initialValue;
			}
			if (constraints[c].fieldName == "CODIGO" || constraints[c].fieldName == "CODCPG") {
				filtro += " AND TCPg.CODCPG = '" + constraints[c].initialValue + "'"
			}
		}
	}

	filtro += " AND TCPg.CODCOLIGADA = '" + CODCOLIGADA + "'"
	context = "CODUSUARIO=svc.totvs.fluig;CODSISTEMA=V;CODCOLIGADA=" + CODCOLIGADA;

	var retorno = dcReadView("MovCPgData", context, usuario, senha, filtro);

    log.info("dsGetCondPGTO - retorno: ");
    log.info(retorno);
    
    var xmlResultados = new XML(retorno);
    var dataset = DatasetBuilder.newDataset();
    
    var registros = xmlResultados.children();
    
    if (registros.length() === 1 && registros[0].children().length() > 0) {
        registros = registros[0].children();
    }
    
    if (registros.length() > 0) {
        var camposPrimeiro = registros[0].children();
    
        for (var c = 0; c < camposPrimeiro.length(); c++) {
            dataset.addColumn(camposPrimeiro[c].name().toString());
        }
    
        for (var i = 0; i < registros.length(); i++) {
            var row = [];
            var campos = registros[i].children();
    
            for (var j = 0; j < campos.length(); j++) {
                row.push(campos[j].toString());
            }
...
function getWebService(Usuario, Senha) {
	...
	var Nome_Servico = "RM DataServer";
