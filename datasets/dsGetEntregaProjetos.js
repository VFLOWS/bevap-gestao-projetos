function createDataset(fields, constraints, sortFields) {
    ...
    if (!baseTable) {
        ...
        newDataset.addRow(["Nao foi possivel localizar a tabela ML do FormEntregaProjetos. Publique o formulario/dataset da entrega e tente novamente."]);
        return newDataset;
    }
...
function resolveBaseTableByDatasetName(datasetNames) {
    var dataSource = "/jdbc/FluigDS";
    var ic = new javax.naming.InitialContext();
    var ds = ic.lookup(dataSource);
    var names = [];

    for (var i = 0; datasetNames && i < datasetNames.length; i++) {
        var name = String(datasetNames[i] || "").replace(/'/g, "''");
        if (name) {
            names.push("'" + name + "'");
        }
    }
    ...
    var query = "SELECT TOP 1 COD_LISTA " +
        "FROM DOCUMENTO " +
        "WHERE VERSAO_ATIVA = 1 " +
        "AND NM_DATASET IN (" + names.join(",") + ") " +
    ...
    try {
        ...
        var rs = stmt.executeQuery(query);
        if (rs.next()) {
            return "ML001" + String(rs.getString("COD_LISTA") || "");
        }
    } catch (e) {
        log.error("$$DEBUG resolveBaseTableByDatasetName ERRO:" + e.message);
    } finally {
        if (stmt != null) {
            stmt.close();
        }
