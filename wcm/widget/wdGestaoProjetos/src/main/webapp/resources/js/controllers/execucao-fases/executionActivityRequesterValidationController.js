const executionActivityRequesterValidationController = {
  _formId: '113669',
  ...
  getTemplateUrl() {
    ...
    return `${baseUrl}/wdGestaoProjetos/resources/js/templates/execucao-fases/ef-activity-requester-validation.html`;
  },

  getContainer() {
    return $('#execution-activity-requester-validation-page');
  },
  ...
  bindEvents() {
    ...
    container.on(`click${ns}`, '#tab-requester-detail', () => this.toggleTab('detail'));
    container.on(`click${ns}`, '#tab-requester-checklist', () => this.toggleTab('checklist'));
    container.on(`scroll${ns}`, '#requester-panel-tabs-scroll', () => this.updateTabArrows());
    container.on(`click${ns}`, '#requester-panel-tabs-left-arrow', () => this.scrollTabsToStart());
    container.on(`click${ns}`, '#requester-panel-tabs-right-arrow', () => this.scrollTabsToEnd());
    container.on(`change${ns}`, '.requester-checklist-item, #requester-agreement-checkbox', () => this.updateChecklistProgress());
    container.on(`click${ns}`, '[data-action="approve-requester"]', () => this.openApproveModal());
  ...
  getDatasetFields() {
    return [
      ...
      'tblRequesterValidationHistoryEF.requesterValidationDescriptionEF',
  ...
  normalizeCard(card) {
    ...
    return {
      ...
      requesterDecision: this.firstFilledValue(source, ['validacaoSolicitante']),
      requesterComment: this.firstFilledValue(source, ['validacaoSolicitanteComentario']),
      requesterChecklist: this.parseChecklistState(source.validacaoSolicitanteChecklist),
      requesterAgreement: this.normalizeBoolean(source.validacaoSolicitanteLiConcordo),
  ...
  render() {
    ...
    this.setText('#ef-req-estimated-hours', this.formatEffort(card.activityEffort) || '-');
    this.setText('#ef-req-project-code', card.projectCode || '-');
    ...
    $('#ef-req-page-alert').addClass('hidden').empty();
    $('#ef-req-content, #ef-req-action-footer').removeClass('hidden');
    setTimeout(() => this.updateTabArrows(), 0);
  },

  renderDependencies(dependencies) {
    const items = Array.isArray(dependencies) ? dependencies.filter(Boolean) : [];
    const block = $('#ef-req-dependencies-block');
    const list = $('#ef-req-dependencies-list');
    if (!items.length) {
  ...
  renderEffortSummary() {
    ...
    $('#ef-req-used-hours').text(this.formatHoursAndMinutes(usedMinutes));
    if (estimatedMinutes) {
      $('#ef-req-estimated-hours').text(this.formatHoursAndMinutes(estimatedMinutes));
      $('#ef-req-remaining-hours').text(this.formatHoursAndMinutes(remainingMinutes));
      $('#ef-req-remaining-summary').removeClass('hidden');
    }
  },

  renderAttachmentsList() {
    const list = $('#requester-files-list');
  ...
  renderChecklistState() {
    const savedItems = this._state.card && Array.isArray(this._state.card.requesterChecklist.items)
      ? this._state.card.requesterChecklist.items
      : [];
    $('.requester-checklist-item').each(function (index, checkbox) {
  ...
  renderHistory() {
    const list = $('#requester-validation-history');
    const history = Array.isArray(this._state.requesterHistory) ? this._state.requesterHistory : [];
    if (!history.length) {
      list.html('<div class="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4 text-sm text-gray-500">Nenhum parecer anterior registrado.</div>');
      return;
    }
    list.html(history.map((entry) => this.getHistoryMarkup(entry)).join(''));
  },
  ...
  toggleTab(tabName) {
    ...
    ['detail', 'checklist'].forEach((tab) => {
      ...
      $(`#tab-content-requester-${tab}`).toggleClass('hidden', !active);
    });
    this.updateChecklistProgress();
  ...
  updateChecklistProgress() {
    const checkboxes = $('.requester-checklist-item');
    const total = checkboxes.length;
    const checked = checkboxes.filter(':checked').length;
    const percent = this.getChecklistProgress(this.collectChecklistState().items);
    $('#requester-checklist-percentage').text(`${percent}%`);
    $('#requester-checklist-progress').css('width', `${percent}%`);
    $('#requester-checklist-notice').toggleClass('hidden', this._state.checklistVisited || checked === total || total === 0);
  },
  ...
  collectChecklistState() {
    return {
      agreement: $('#requester-agreement-checkbox').is(':checked'),
      items: $('.requester-checklist-item').map(function (_, checkbox) {
  ...
  async submitDecision(config) {
    ...
    try {
      ...
      const taskFields = this.collectValidationTaskFields({
        decision: config.decision,
        comment: $('#requester-feedback-text').val(),
      ...
      await fluigService.saveAndSendTask({
        ...
        comments: this.asText(config.comments)
      }, taskFields);
  ...
  validateDecisionInput(input) {
    ...
    if (input && input.requireDescription && !description) {
      return { valid: false, agreementOk: true, message: 'Informe a justificativa.', description, category };
    }
    return { valid: true, agreementOk: true, description, category };
  },

  collectValidationTaskFields(config) {
    const checklist = config && config.checklist ? config.checklist : { agreement: false, items: [] };
    const historyEntry = this.buildRequesterValidationHistoryEntry(config || {});
    const rowIndex = this.getNextRequesterValidationHistoryIndex(config && config.history);

    return [
      { name: 'validacaoSolicitante', value: this.asText(config && config.decision) },
      { name: 'validacaoSolicitanteComentario', value: this.asText(config && config.comment) },
      { name: 'validacaoSolicitanteDescricao', value: this.asText(config && config.description) },
      { name: 'validacaoSolicitanteCategoria', value: this.asText(config && config.category) },
      { name: 'validacaoSolicitanteChecklist', value: JSON.stringify(checklist) },
      { name: 'validacaoSolicitanteLiConcordo', value: checklist.agreement ? 'true' : 'false' },
      { name: `requesterValidationIdEF___${rowIndex}`, value: historyEntry.id },
      { name: `requesterValidationDecisionEF___${rowIndex}`, value: historyEntry.decision },
      { name: `requesterValidationCommentEF___${rowIndex}`, value: historyEntry.comment },
      { name: `requesterValidationDescriptionEF___${rowIndex}`, value: historyEntry.description },
      { name: `requesterValidationCategoryEF___${rowIndex}`, value: historyEntry.category },
      { name: `requesterValidationUserIdEF___${rowIndex}`, value: historyEntry.userId },
      { name: `requesterValidationUserNameEF___${rowIndex}`, value: historyEntry.userName },
      { name: `requesterValidationCreatedAtEF___${rowIndex}`, value: historyEntry.createdAt }
    ];
  },

  buildRequesterValidationHistoryEntry(data) {
    const now = new Date().toISOString();
    return {
      id: this.asText(data && data.id) || this.createRequesterValidationHistoryId(now),
  ...
  createRequesterValidationHistoryId(now) {
    const timestamp = this.asText(now) || new Date().toISOString();
    return `requester-validation-${timestamp.replace(/[^0-9]/g, '')}`;
  },

  getNextRequesterValidationHistoryIndex(history) {
  ...
  parseRequesterValidationHistory(card) {
    const source = card || {};
    const tableRows = this.parseTableJson(source.tblRequesterValidationHistoryEF);
    const rowsByIndex = {};
    tableRows.forEach((row, index) => this.mergeRequesterValidationHistoryRow(rowsByIndex, row, row.rowIndex || row.index || index + 1));
    this.extractIndexedRows(source, this.getRequesterValidationHistoryFieldNames())
      .forEach((row, index) => this.mergeRequesterValidationHistoryRow(rowsByIndex, row, row.rowIndex || index + 1));

    const rows = Object.keys(rowsByIndex)
      .sort((a, b) => Number(a) - Number(b))
      .map((idx) => this.normalizeRequesterValidationHistoryRow(rowsByIndex[idx] || {}, idx))
      .filter((entry) => this.hasRequesterValidationHistoryPayload(entry));
  ...
  getRequesterValidationHistoryFieldNames() {
    return [
      ...
      'requesterValidationDescriptionEF',
  ...
  mergeRequesterValidationHistoryRow(rowsByIndex, row, fallbackIndex) {
    if (!row) return;
    const rowIndex = Number(row.rowIndex) || Number(fallbackIndex) || 1;
    rowsByIndex[rowIndex] = rowsByIndex[rowIndex] || { rowIndex };
    this.getRequesterValidationHistoryFieldNames().forEach((fieldName) => {
  ...
  normalizeRequesterValidationHistoryRow(row, idx) {
    ...
    return {
      ...
      description: this.asText(row.requesterValidationDescriptionEF),
  ...
  hasRequesterValidationHistoryPayload(entry) {
    return Boolean(
  ...
  updateTabArrows() {
    const scroller = document.getElementById('requester-panel-tabs-scroll');
    const leftArrow = document.getElementById('requester-panel-tabs-left-arrow');
    const rightArrow = document.getElementById('requester-panel-tabs-right-arrow');
  ...
  renderError(message) {
    $('#ef-req-content, #ef-req-action-footer').addClass('hidden');
    $('#ef-req-page-alert').removeClass('hidden').html(`
  ...
  setActionButtonsState(isDisabled) {
    $('[data-action="approve-requester"], [data-action="return-corrections"], [data-action="stop-flow"]')
...
if (typeof window !== 'undefined') {
  window.executionActivityRequesterValidationController = executionActivityRequesterValidationController;
}
