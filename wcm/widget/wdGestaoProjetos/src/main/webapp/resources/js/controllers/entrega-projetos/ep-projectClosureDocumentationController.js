const epProjectClosureDocumentationController = {
  _eventNamespace: '.epProjectClosureDocumentation',
  ...
  load: async function (params) {
    ...
    try {
      ...
      console.error('[epProjectClosureDocumentation] load error:', error);
      $('#page-container').html('<div class="p-6 text-red-600">Falha ao carregar a documentacao de encerramento.</div>');
    }
  },
  ...
  getTemplateUrl: function () {
    return WCMAPI.getServerURL() + '/wdGestaoProjetos/resources/js/templates/entrega-projetos/ep-project-closure-documentation.html';
  },
  ...
  backupAndSetHeader: function () {
    ...
    title.text('TI - Anexar Documentacao de Encerramento do Projeto');
    breadcrumb.html([
      ...
      '<span class="text-gray-400">/</span><span class="font-medium text-bevap-gold">Encerramento</span>'
    ].join(''));
  },
  ...
  bindEvents: function () {
    ...
    $(window).off(this._eventNamespace).on('resize' + this._eventNamespace, function () { self.refreshTabsArrows(); });

    container.on('scroll' + this._eventNamespace, '#ep-closure-tabs-scroll', function () { self.refreshTabsArrows(); });
    container.on('click' + this._eventNamespace, '#ep-closure-tabs-left-arrow', function () { self.scrollTabs(-1); });
    container.on('click' + this._eventNamespace, '#ep-closure-tabs-right-arrow', function () { self.scrollTabs(1); });
    container.on('click' + this._eventNamespace, '#ep-closure-overview-tab', function () { self.setTab('overview'); });
    container.on('click' + this._eventNamespace, '#ep-closure-requester-tab', function () { self.setTab('requester'); });
    container.on('click' + this._eventNamespace, '#ep-closure-final-tab', function () { self.setTab('final'); });
    container.on('click' + this._eventNamespace, '#ep-closure-golive-tab', function () { self.setTab('golive'); });
    container.on('click' + this._eventNamespace, '[data-action="open-closure-documents"]', function () { self.setTab('overview'); });
    container.on('click' + this._eventNamespace, '[data-action="set-closure-plan-status"]', function () {
      self.openStatusModal($(this).attr('data-plan-id'), $(this).attr('data-status'));
    });
    container.on('click' + this._eventNamespace, '[data-action="close-closure-status"]', function () { self.closeModal('#ep-closure-status-modal'); self._state.pendingStatusChange = null; });
    container.on('click' + this._eventNamespace, '[data-action="confirm-closure-status"]', function () { self.confirmStatusChange(); });
    container.on('change' + this._eventNamespace, '#ep-closure-documents-input', function () { self.addDocuments(this.files); this.value = ''; });
    container.on('dragover' + this._eventNamespace, '#ep-closure-documents-dropzone', function (event) {
      event.preventDefault();
      $(this).addClass('border-bevap-green bg-green-50');
    });
    container.on('dragleave' + this._eventNamespace + ' drop' + this._eventNamespace, '#ep-closure-documents-dropzone', function (event) {
      event.preventDefault();
    ...
    container.on('click' + this._eventNamespace, '[data-action="remove-closure-document"]', function () {
      self._state.documents.splice(parseInt($(this).attr('data-document-index'), 10), 1);
      self.renderDocuments();
    });
    container.on('click' + this._eventNamespace, '[data-action="save-closure-draft"]', function () { self.saveDraft(); });
    container.on('click' + this._eventNamespace, '[data-action="open-closure-return"]', function () { self.openModal('#ep-closure-return-modal'); });
    container.on('click' + this._eventNamespace, '[data-action="close-closure-return"]', function () { self.closeModal('#ep-closure-return-modal'); });
    container.on('click' + this._eventNamespace, '[data-action="confirm-closure-return"]', function () { self.confirmReturn(); });
    container.on('click' + this._eventNamespace, '[data-action="open-closure-cancel"]', function () { self.openModal('#ep-closure-cancel-modal'); });
    container.on('click' + this._eventNamespace, '[data-action="close-closure-cancel"]', function () { self.closeModal('#ep-closure-cancel-modal'); });
    container.on('click' + this._eventNamespace, '[data-action="confirm-closure-cancel"]', function () { self.confirmCancel(); });
    container.on('click' + this._eventNamespace, '[data-action="open-closure-approve"]', function () { self.openApproveModal(); });
    container.on('click' + this._eventNamespace, '[data-action="close-closure-approve"]', function () { self.closeModal('#ep-closure-approve-modal'); });
    container.on('click' + this._eventNamespace, '[data-action="confirm-closure-approve"]', function () { self.submitDecision('aprovado', '', '', 'Documentacao de encerramento concluida via Widget'); });
  },
  ...
  loadData: async function () {
    ...
    this._state.documents = this.parsePersistedAttachments(this.getValIgnoreCase(row, 'anexosEncerramentoEP'));
    ...
    this._state.closureHistory = this.normalizeHistory(this.parseJson(this.getValIgnoreCase(row, 'histEncerramentoEP')));
    $('#ep-closure-opinion').val(this.asText(this.getValIgnoreCase(row, 'comentEncerramentoEP')));
    $('#ep-closure-agreement').prop('checked', this.asText(this.getValIgnoreCase(row, 'confirmaEncerramentoEP')) === 'true');
  },
  ...
  renderAll: function () {
    ...
    this.updateTabs();
    this.refreshTabsArrows();
  },

  renderProjectSummary: function () {
    var summary = this._state.projectSummary || {};
    $('#ep-closure-project-code').text(summary.code || '-');
    $('#ep-closure-project-title').text(summary.title || '-');
    $('#ep-closure-project-requester').text(summary.requester || '-');
    $('#ep-closure-approve-message').html('Voce esta confirmando o encerramento do projeto <strong>' + this.escapeHtml((summary.code || '-') + ' - ' + (summary.title || '-')) + '</strong>.');
  },

  renderProgress: function () {
    $('#ep-closure-progress-list').html([
      ...
      '<div class="flex items-center text-green-600"><i class="fa-solid fa-check-circle mr-2"></i><span>Validacao final TI concluida</span></div>',
      ...
      '<div class="flex items-center text-green-600"><i class="fa-solid fa-check-circle mr-2"></i><span>Solicitante - Validar GO Live concluido</span></div>',
      '<div class="flex items-center text-bevap-gold"><i class="fa-solid fa-clock mr-2"></i><span>TI - Anexar Documentacao de Encerramento</span></div>'
    ].join(''));
  },

  renderPlanningCards: function () {
    var self = this;
    $('#ep-closure-planning-list').html((this._state.deliveryPlans || []).map(function (plan) {
  ...
  getPlanningCardHtml: function (plan) {
    ...
    var showButtons = this.canSetPlanStatus(plan);
    ...
    var buttons = showButtons ? [
      ...
      '<button type="button" data-action="set-closure-plan-status" data-plan-id="' + this.escapeHtml(plan.id) + '" data-status="realizado" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium ' + realizedButtonClasses + '"><i class="fa-solid fa-circle-check"></i>Realizado</button>',
      '<button type="button" data-action="set-closure-plan-status" data-plan-id="' + this.escapeHtml(plan.id) + '" data-status="nao_realizado" class="inline-flex items-center gap-1.5 border-l border-gray-200 px-3 py-1.5 text-xs font-medium ' + notRealizedButtonClasses + '"><i class="fa-solid fa-ban"></i>Nao Realizado</button>',
      '</span>'
    ].join('') : '';
    return [
      '<div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">',
      '<div class="flex items-start justify-between gap-4"><div class="flex min-w-0 items-center gap-3"><span class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-blue-100 text-blue-700"><i class="fa-solid fa-rocket"></i></span><div><h3 class="text-base font-montserrat font-semibold text-bevap-navy">' + this.escapeHtml(plan.title) + '</h3><p class="mt-1 text-sm text-gray-500">Responsavel: ' + this.escapeHtml(plan.responsible || '-') + '</p></div></div><div class="flex items-center gap-2">' + this.getPlanningStatusBadge(status) + '</div></div>',
  ...
  renderHistoryTabs: function () {
    $('#ep-closure-requester-history-list').html(this.getHistoryListHtml(this._state.requesterHistory, 'Validado'));
    $('#ep-closure-final-history-list').html(this.getHistoryListHtml(this._state.finalHistory, 'Validado'));
    $('#ep-closure-golive-history-list').html(this.getHistoryListHtml(this._state.goLiveHistory, 'Concluido'));
  },
  ...
  renderDocuments: function () {
    $('#ep-closure-documents-list').html(this.getDocumentsHtml());
  },

  getDocumentsHtml: function () {
    var self = this;
    return (this._state.documents || []).map(function (doc, index) {
      var remove = doc.persisted ? '<i class="fa-solid fa-lock text-gray-300"></i>' : '<button type="button" data-action="remove-closure-document" data-document-index="' + index + '" class="text-gray-400 hover:text-red-500" title="Remover"><i class="fa-solid fa-trash"></i></button>';
  ...
  setTab: function (tab) {
    this._state.currentTab = ['requester', 'final', 'golive'].indexOf(tab) >= 0 ? tab : 'overview';
    this.updateTabs();
  },
  ...
  updateTabs: function () {
    ...
    var tabs = [
      { name: 'overview', tab: '#ep-closure-overview-tab', content: '#ep-closure-overview-content' },
      { name: 'requester', tab: '#ep-closure-requester-tab', content: '#ep-closure-requester-content' },
      { name: 'final', tab: '#ep-closure-final-tab', content: '#ep-closure-final-content' },
      { name: 'golive', tab: '#ep-closure-golive-tab', content: '#ep-closure-golive-content' }
    ];
    var self = this;
    tabs.forEach(function (item) {
      var active = item.name === current;
      $(item.tab).attr('class', active ? 'shrink-0 whitespace-nowrap border-b-2 border-bevap-green bg-green-50 px-6 py-4 text-sm font-medium text-bevap-green' : 'shrink-0 whitespace-nowrap border-b-2 border-transparent px-6 py-4 text-sm font-medium text-gray-500 hover:text-gray-700');
      $(item.content).toggleClass('hidden', !active);
      if (active) self.scrollTabIntoView($(item.tab)[0]);
    });
    setTimeout(function () { self.refreshTabsArrows(); }, 200);
  },

  scrollTabs: function (direction) {
    var tabsScroll = $('#ep-closure-tabs-scroll')[0];
    if (!tabsScroll) return;
    var distance = Math.max(280, Math.floor(tabsScroll.clientWidth * 0.85));
    tabsScroll.scrollBy({ left: direction * distance, behavior: 'smooth' });
  },

  scrollTabIntoView: function (tabButton) {
    var tabsScroll = $('#ep-closure-tabs-scroll')[0];
    ...
    var padding = 48;
    if (tabLeft < visibleLeft + padding) tabsScroll.scrollTo({ left: Math.max(0, tabLeft - padding), behavior: 'smooth' });
    else if (tabRight > visibleRight - padding) tabsScroll.scrollTo({ left: tabRight - tabsScroll.clientWidth + padding, behavior: 'smooth' });
  },

  refreshTabsArrows: function () {
    var tabsScroll = $('#ep-closure-tabs-scroll')[0];
    var leftArrow = $('#ep-closure-tabs-left-arrow');
    var rightArrow = $('#ep-closure-tabs-right-arrow');
    if (!tabsScroll || !leftArrow.length || !rightArrow.length) return;
    ...
    leftArrow.toggleClass('opacity-0 pointer-events-none', !canScrollLeft);
    rightArrow.toggleClass('opacity-0 pointer-events-none', !canScrollRight);
  },

  canSetPlanStatus: function (plan) {
    return plan && plan.type !== 'treinamento' && this.asText(plan.goLiveStatus || 'planejado') === 'planejado';
  },

  openStatusModal: function (planId, status) {
    var plan = this.findPlanById(planId);
    if (!this.canSetPlanStatus(plan)) return;
    this._state.pendingStatusChange = { planId: planId, status: status };
    var realized = status === 'realizado';
    $('#ep-closure-status-title').text(realized ? 'Confirmar Planejamento Realizado' : 'Confirmar Planejamento Nao Realizado');
    $('#ep-closure-status-message').text('Deseja atualizar "' + (plan.title || 'Planejamento') + '" para o status ' + (realized ? 'Realizado' : 'Nao Realizado') + '?');
    this.openModal('#ep-closure-status-modal');
  },
  ...
  confirmStatusChange: function () {
    ...
    if (plan) plan.goLiveStatus = pending.status;
    this._state.pendingStatusChange = null;
    this.closeModal('#ep-closure-status-modal');
    this.renderPlanningCards();
  ...
  openApproveModal: function () {
    ...
    if (!validation.valid) return this.showToast('Validacao pendente', validation.message, 'warning');
    this.openModal('#ep-closure-approve-modal');
  },

  confirmReturn: function () {
    var required = this.validateRequiredConfirmation();
    if (!required.valid) return this.showToast('Validacao pendente', required.message, 'warning');
    var reason = this.asText($('#ep-closure-return-reason').val());
    if (!reason) return this.showToast('Informe o motivo', 'Descreva o motivo do novo planejamento.', 'warning');
    this.closeModal('#ep-closure-return-modal');
    this.submitDecision('correcao', reason, '', 'Encerramento devolvido para novo planejamento via Widget');
  },

  confirmCancel: function () {
    var required = this.validateRequiredConfirmation();
    if (!required.valid) return this.showToast('Validacao pendente', required.message, 'warning');
    var category = this.asText($('#ep-closure-cancel-category').val());
    var reason = this.asText($('#ep-closure-cancel-reason').val());
    ...
    this.closeModal('#ep-closure-cancel-modal');
    this.submitDecision('cancelado', reason, category, 'Nao continuidade do encerramento registrada via Widget');
  },

  validateRequiredConfirmation: function () {
    if (!this.asText($('#ep-closure-opinion').val())) return { valid: false, message: 'Informe o comentario final da TI.' };
    if (!$('#ep-closure-agreement').is(':checked')) return { valid: false, message: 'Marque a confirmacao da documentacao de encerramento.' };
  ...
  saveDraft: async function () {
    ...
    try {
      ...
      try { sessionStorage.setItem('gpDashboardFeedback', JSON.stringify({ title: 'Rascunho salvo', message: 'A documentacao de encerramento foi salva com sucesso.', type: 'success' })); } catch (ignore) {}
      location.hash = '#dashboard';
    } catch (error) {
      console.error('[epProjectClosureDocumentation] saveDraft error:', error);
  ...
  submitDecision: async function (decision, reason, category, comments) {
    ...
    var loading = modalLoadingService.show({
      title: 'Registrando documentacao',
    ...
    try {
      ...
      loading.updateMessage('Preparando anexos da documentacao...');
      ...
      if (window.gpActionFeedback && typeof window.gpActionFeedback.showProcessSuccess === 'function') {
        window.gpActionFeedback.showProcessSuccess({
          ...
          message: 'Documentacao de encerramento registrada com sucesso.',
          nextStep: 'Acompanhe o encerramento pelo dashboard.'
        });
      } else {
        this.showToast('Sucesso', 'Documentacao de encerramento registrada com sucesso.', 'success');
      }
    } catch (error) {
      console.error('[epProjectClosureDocumentation] submitDecision error:', error);
  ...
  collectTaskFields: function (decision, reason, category, includePending) {
    ...
    fields.push(
      { name: 'decisaoEncerramentoEP', value: this.asText(decision) },
      { name: 'comentEncerramentoEP', value: this.asText($('#ep-closure-opinion').val()) },
      { name: 'confirmaEncerramentoEP', value: $('#ep-closure-agreement').is(':checked') ? 'true' : 'false' },
      { name: 'justifEncerramentoEP', value: this.asText(reason) },
      { name: 'catCancelEncerrEP', value: this.asText(category) },
      { name: 'anexosEncerramentoEP', value: JSON.stringify(this.buildAttachmentMetadata(includePending)) },
      { name: 'histEncerramentoEP', value: JSON.stringify(this.buildHistory(decision, reason, category, includePending)) }
    );
    return fields;
  },
  ...
  buildHistory: function (decision, reason, category, appendEntry) {
    var history = this.normalizeHistory(this._state.closureHistory);
    if (!appendEntry || !this.asText(decision)) return history;
    history.push({
      decision: this.asText(decision),
      comment: this.asText($('#ep-closure-opinion').val()),
