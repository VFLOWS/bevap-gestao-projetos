const epFinalGoLiveValidationController = {
  ...
  load: async function (params) {
    ...
    try {
      ...
      $('#page-container').html('<div class="p-6 text-red-600">Falha ao carregar a validacao final do GO Live.</div>');
    }
  },
  ...
  backupAndSetHeader: function () {
    ...
    title.text('TI - Validacao Final do Projeto para GO Live');
    breadcrumb.html([
      ...
      '<span class="text-gray-400">/</span><span class="font-medium text-bevap-gold">Validacao Final GO Live</span>'
    ].join(''));
  },
  ...
  bindEvents: function () {
    ...
    container.on('click' + this._eventNamespace, '[data-action="confirm-final-approve"]', function () { self.submitDecision('aprovado', '', '', 'Validacao final concluida via Widget'); });
  },
  ...
  renderProjectSummary: function () {
    ...
    $('#ep-final-approve-message').html('Voce esta confirmando a validacao final da TI para o projeto <strong>' + this.escapeHtml((summary.code || '-') + ' - ' + (summary.title || '-')) + '</strong>.');
  },
  ...
  renderProgress: function () {
    $('#ep-final-progress-list').html([
      ...
      '<div class="flex items-center text-bevap-gold"><i class="fa-solid fa-clock mr-2"></i><span>TI - Validacao final do projeto para GO Live</span></div>'
    ].join(''));
  },
  ...
  confirmReturn: function () {
    ...
    this.submitDecision('correcao', reason, '', 'Validacao final devolvida para novo planejamento via Widget');
  },
  ...
  saveDraft: async function () {
    ...
    try {
      ...
      try { sessionStorage.setItem('gpDashboardFeedback', JSON.stringify({ title: 'Rascunho salvo', message: 'A validacao final foi salva com sucesso.', type: 'success' })); } catch (ignore) {}
  ...
  submitDecision: async function (decision, reason, category, comments) {
    ...
    try {
      ...
      loading.updateMessage('Preparando anexos da validacao final...');
      ...
      if (window.gpActionFeedback && typeof window.gpActionFeedback.showProcessSuccess === 'function') {
        window.gpActionFeedback.showProcessSuccess({
          ...
          message: 'Validacao final registrada com sucesso.',
          nextStep: 'Acompanhe a proxima etapa da entrega pelo dashboard.'
        });
      } else {
        this.showToast('Sucesso', 'Validacao final registrada com sucesso.', 'success');
      }
    } catch (error) {
