const epDeliveryPlanningController = {
  ...
  confirmConclude: async function () {
    ...
    try {
      ...
      loading = modalLoadingService.show({
        title: 'Concluindo planejamento',
        message: 'Preparando dados da entrega...'
      });
      await loading.waitForPaint();
      var result = await this.submit(loading);
      loading.hide();
      if (window.gpActionFeedback && typeof window.gpActionFeedback.showProcessSuccess === 'function') {
        window.gpActionFeedback.showProcessSuccess({
          controller: this,
          processInstanceId: result && result.processInstanceId,
          documentId: result && result.documentId,
          title: 'Planejamento concluido!',
          message: 'O planejamento da entrega foi consolidado com sucesso.',
  ...
  submit: async function (loading) {
    var documentId = this.asText(this._state.documentId);
    if (!documentId) throw new Error('documentId nao informado.');
    if (loading && loading.updateMessage) loading.updateMessage('Localizando processo da entrega...');
    var processInstanceId = await fluigService.resolveProcessInstanceIdByDocumentId(documentId);
    if (loading && loading.updateMessage) loading.updateMessage('Preparando anexos da entrega...');
    ...
    if (loading && loading.updateMessage) loading.updateMessage('Enviando movimentacao para o Fluig...');
