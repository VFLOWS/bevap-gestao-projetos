  ...
  async ensureActionAccess(params = {}) {
    if (typeof fluigService === 'undefined' || !fluigService.resolveProjectActionAccess) {
      ...
      if (typeof router !== 'undefined' && router.showActionAccessDeniedModal) {
        router.showActionAccessDeniedModal('Não foi possível validar seu acesso a esta solicitação.');
      }
      return false;
    }

    console.log('[GP][executionActivityWaitingAccess] validando acesso:', params);

    const access = await fluigService.resolveProjectActionAccess('executionActivityWaiting', params || {});
    ...
    const message = access && access.message ? access.message : 'Você não tem permissão para atuar nessa solicitação.';
    if (typeof router !== 'undefined' && router.showActionAccessDeniedModal) {
