...
var fluigService = {
    ...
    resolveProjectActionAccess: async function (routeName, params) {
        ...
        project.processType = this.asTrimmedString(routeContext.processType || project.processType);
        project.processName = this.asTrimmedString(routeContext.processName || project.processName);
        project.estadoProcesso = this.asTrimmedString(
            routeContext.activity !== null && routeContext.activity !== undefined
                ? routeContext.activity
                : (p.estadoProcesso || p.activity || project.estadoProcesso)
        );
        project.activity = routeContext.activity !== null && routeContext.activity !== undefined
            ? routeContext.activity
            : (
                project.activity !== null && project.activity !== undefined
                    ? project.activity
                    : this.parseProjectProcessActivity(p.activity || p.estadoProcesso)
            );
        ...
        var canAct = dashboardController.canCurrentUserActOnPendency(dashboardProject, accessContext);
