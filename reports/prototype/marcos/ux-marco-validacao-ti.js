document.addEventListener('DOMContentLoaded', function () {
    ...
    function toggleTab(tabPrefix, tabName) {
        ['detail', 'requester', 'checklist'].forEach(function (tab) {
    ...
    function updateTiChecklistProgress() {
        var items = document.querySelectorAll('.ti-checklist-item');
        var checked = document.querySelectorAll('.ti-checklist-item:checked').length;
        var percentage = items.length ? Math.round((checked / items.length) * 100) : 0;
        var percentageElement = document.getElementById('ti-checklist-percentage');
        var progressElement = document.getElementById('ti-checklist-progress');
        var tabNotice = document.getElementById('ti-checklist-notice');
        var arrowNotice = document.getElementById('ti-checklist-arrow-notice');
        ...
        if (tabNotice) tabNotice.classList.toggle('hidden', !hasPendingItems);
        if (arrowNotice) {
            arrowNotice.classList.toggle('hidden', !hasPendingItems);
            arrowNotice.classList.toggle('flex', hasPendingItems);
        }
    }

    function validateTiApprovalRequirements() {
        var agreement = document.getElementById('ti-agreement-checkbox');
        var checklistItems = document.querySelectorAll('.ti-checklist-item');
        var checkedCount = document.querySelectorAll('.ti-checklist-item:checked').length;
        var allChecklistDone = checklistItems.length > 0 && checkedCount === checklistItems.length;

        if (!allChecklistDone) {
            showToast('Checklist pendente', 'Conclua o checklist da TI antes de confirmar a validação.', 'error');
            toggleTab('ti', 'checklist');
            return false;
        }

        if (!agreement || !agreement.checked) {
            showToast('Confirmação pendente', 'Marque a confirmação da TI antes de concluir a validação.', 'error');
            document.getElementById('ti-feedback-text')?.focus();
            return false;
        }
    ...
    bindTabs('ti', 'ti-panel');
    updateTiChecklistProgress();
    document.querySelectorAll('.ti-checklist-item').forEach(function (checkbox) {
        checkbox.addEventListener('change', updateTiChecklistProgress);
    });
    document.getElementById('ti-agreement-checkbox')?.addEventListener('change', updateTiChecklistProgress);

    document.querySelector('[data-action="approve-ti"]')?.addEventListener('click', function () {
        if (!validateTiApprovalRequirements()) return;
    ...
    document.querySelector('[data-action="confirm-approve-modal"]')?.addEventListener('click', function () {
        closeModal('approve-modal');
        showToast('Validação concluída', 'A validação técnica foi registrada com sucesso.', 'success');
