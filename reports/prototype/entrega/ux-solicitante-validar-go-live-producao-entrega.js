(function () {
    ...
    function updateTabs() {
        ...
        if (!overviewTab || !finalValidationTab || !validationTab || !documentsTab || !overviewContent || !finalValidationContent || !validationContent || !documentsContent) return;
    ...
    function setCurrentTab(tabName) {
        currentTab = tabName === 'documents' || tabName === 'validation' || tabName === 'final-validation' ? tabName : 'overview';
        updateTabs();
    }
    ...
    function bindEvents() {
        document.addEventListener('click', function (event) {
            ...
            if (concludeButton) {
                ...
                if (!finalNote.trim()) {
                    showToast('Informe a observação final', 'Registre a observação final do solicitante antes de concluir a validação do GO Live em produção.', 'error');
                    return;
                }
                if (!agreementCheckbox || !agreementCheckbox.checked) {
                    showToast('Confirmação obrigatória', 'Marque a confirmação da validação do solicitante antes de concluir esta etapa.', 'error');
                    return;
                }
            ...
            if (saveButton) {
                showToast('Rascunho salvo', 'O progresso da validação do GO Live em produção foi salvo com sucesso.', 'success');
            }
        });
        ...
        if (returnConfirmButton) {
            returnConfirmButton.addEventListener('click', function () {
                ...
                showToast('Retorno solicitado', 'A validação foi direcionada para novo planejamento do GO Live.', 'info');
            });
        }
        ...
        if (discontinueConfirmButton) {
            discontinueConfirmButton.addEventListener('click', function () {
                ...
                showToast('Não continuidade registrada', 'A não continuidade da validação do GO Live em produção foi registrada com sucesso.', 'info');
            });
        }
        ...
        if (concludeConfirmButton) {
            concludeConfirmButton.addEventListener('click', function () {
                closeModal('conclude-modal');
                showToast('Validação do solicitante concluída', 'O GO Live em produção foi validado com sucesso pelo solicitante.', 'success');
            });
        }
    }
