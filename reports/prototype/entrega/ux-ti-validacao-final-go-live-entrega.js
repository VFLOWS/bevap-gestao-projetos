(function () {
    ...
    var finalValidationDocumentsData = [
        { name: 'Checklist_Liberacao_Producao.pdf', meta: '760 KB' },
        { name: 'Plano_GoLive_vFinal.pdf', meta: '1.2 MB' },
        { name: 'Ata_Validacao_TI.docx', meta: '320 KB' }
    ];
    ...
    function showToast(title, message, type) {
        ...
        window.clearTimeout(window.__tiValidationToastTimeout);
        window.__tiValidationToastTimeout = window.setTimeout(function () {
    ...
    function updateTabs() {
        var overviewTab = document.getElementById('tab-ti-final-overview');
        var documentsTab = document.getElementById('tab-ti-final-documents');
        var overviewContent = document.getElementById('tab-content-ti-final-overview');
        var documentsContent = document.getElementById('tab-content-ti-final-documents');
        if (!overviewTab || !documentsTab || !overviewContent || !documentsContent) return;
        ...
        documentsTab.className = !isOverview
            ? 'border-b-2 border-bevap-green bg-green-50 px-6 py-4 text-sm font-medium text-bevap-green'
            : 'border-b-2 border-transparent px-6 py-4 text-sm font-medium text-gray-500 hover:text-gray-700';

        overviewContent.classList.toggle('hidden', !isOverview);
        documentsContent.classList.toggle('hidden', isOverview);
    }

    function setCurrentTab(tabName) {
        currentTab = tabName === 'documents' ? 'documents' : 'overview';
        updateTabs();
    }

    function renderPlanningCards() {
        var container = document.getElementById('ti-go-live-overview-list');
        if (!container) return;
        ...
        container.innerHTML = '' +
            '<div class="bg-white p-5">' +
                ...
                '<div class="mt-4 space-y-3">' +
                    finalValidationDocumentsData.map(function (attachment) {
                        return createAttachmentItemHTML(attachment);
                    }).join('') +
                '</div>' +
            '</div>';
    ...
    function bindEvents() {
        document.addEventListener('click', function (event) {
            ...
            var documentsTabButton = event.target.closest('#tab-ti-final-documents');
            ...
            var discontinueButton = event.target.closest('[data-action="discontinue-ti-validation"]');
            if (discontinueButton) {
                openModal('discontinue-modal');
                return;
            }

            var concludeButton = event.target.closest('[data-action="conclude-ti-validation"]');
            if (concludeButton) {
                var agreementCheckbox = document.getElementById('ti-final-agreement-checkbox');
                if (!finalOpinion.trim()) {
                    showToast('Informe o comentário final', 'Registre a observação final da TI antes de concluir esta etapa.', 'error');
                    return;
                }
                if (!agreementCheckbox || !agreementCheckbox.checked) {
                    showToast('Confirmação obrigatória', 'Marque a confirmação da validação técnica final antes de concluir esta etapa.', 'error');
                    return;
                }
            ...
            var saveButton = event.target.closest('[data-action="save-ti-validation-progress"]');
            if (saveButton) {
                showToast('Rascunho salvo', 'O progresso da validação final da TI foi salvo com sucesso.', 'success');
            }
        });

        document.addEventListener('input', function (event) {
            if (event.target.id === 'ti-final-opinion') {
                finalOpinion = event.target.value;
            }
        });
        ...
        if (returnConfirmButton) {
            returnConfirmButton.addEventListener('click', function () {
                ...
                showToast('Retorno solicitado', 'A entrega foi direcionada para novo planejamento do GO Live.', 'info');
            });
        }
        if (discontinueCancelButton) discontinueCancelButton.addEventListener('click', function () {
        ...
        if (discontinueConfirmButton) {
            discontinueConfirmButton.addEventListener('click', function () {
                ...
                showToast('Não continuidade registrada', 'A não continuidade da entrega foi registrada com sucesso.', 'info');
            });
        }
        if (concludeCancelButton) concludeCancelButton.addEventListener('click', function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        var finalOpinionField = document.getElementById('ti-final-opinion');
        if (finalOpinionField) {
            finalOpinion = finalOpinionField.value || '';
        }
        renderPlanningCards();
        renderDocumentsTab();
        updateTabs();
